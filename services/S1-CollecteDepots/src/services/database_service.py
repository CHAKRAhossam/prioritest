...
class DatabaseService:
    ...
    def list_repositories(self, source: str = None) -> list:
        ...
        with self.get_session() as session:
            ...
            repos = query.all()
            # Serialize data within session to avoid detached instance errors
            result = []
            for repo in repos:
                result.append({
                    "id": repo.id,
                    ...
                    "metadata_json": repo.metadata_json or {}
                })
            logger.info(f"Listed {len(result)} repositories" + (f" for source {source}" if source else ""))
            return result
